...
async function setupAdmin() {
  ...
  try {
    ...
    if (url.includes('/setup-database')) {
      console.log('On database setup page.');

      try {
        // Select SQLite by clicking the label
        console.log('Selecting SQLite...');
        await page.click('label[for="btnradio1"]');

        // Wait for button to be enabled
        const btn = await page.waitForSelector('button[type="submit"]:not([disabled])', { timeout: 5000 });
        await btn.click();
        console.log('Submitted database setup.');
      } catch (e) {
        console.log('Error selecting database:', e);
        await page.screenshot({ path: 'uptime-kuma-db-setup-error.png' });
        throw e;
      }

      // Wait for navigation to setup or dashboard (if using default admin?)
      // Usually it goes to /setup or similar
      await page.waitForURL('**/setup', { timeout: 30000 });
      console.log('Navigated to admin setup.');
